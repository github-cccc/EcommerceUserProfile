 ...
 * @Author: github_zhu
 * @Describtion:
 * @Date:Created in 2020/6/11 9:25
 * @ModifiedBy:
 */
public class WowEtl {
    ...
    private static List<EveryDayRegVo> regWeekCountEtl(SparkSession session) {
        LocalDate now = LocalDate.of(2019, 11, 30);
       ...
       /* String regCountSql = "select date_format(create_time, 'yyyy-MM-dd') as day, " +
        ...
        regCountSql = String.format(regCountSql, DateStyle.YYYY_MM_DD_HH_MM_SS, DateUtil.DateToString(nowDay, DateStyle.YYYY_MM_DD_HH_MM_SS));
    ...
    private static List<EveryDayOrderVo> orderWeekCountEtl(SparkSession session) {
        ...
        String orderCountSql = "select date_format(create_time, 'yyyy-MM-dd') as day, " +
            ...
            /*
                ...
                "count(order_id) as regCount from ecommerce.t_order" +
                "where create_time >= '%s'" +
                "group by date_format(create_time,'yyyy-MM-dd') order by day";*/

        orderCountSql = String.format(orderCountSql, DateStyle.YYYY_MM_DD_HH_MM_SS, DateUtil.DateToString(nowDay, DateStyle.YYYY_MM_DD_HH_MM_SS));
